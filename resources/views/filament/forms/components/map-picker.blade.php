<x-dynamic-component :component="$getFieldWrapperView()" :field="$field">
    <div
        ...
        x-init="(async () => {
            ...
            if (!key) { error = 'Peta belum tersedia. Isi koordinat secara manual.'; loading = false; return; }
            ...
            const latitude = $wire.get('data.latitude');
            const longitude = $wire.get('data.longitude');
            const center = hasCoordinates(latitude) && hasCoordinates(longitude)
            ...
            const syncMarker = () => {
                const lat = $wire.get('data.latitude');
                const lng = $wire.get('data.longitude');
                if (!hasCoordinates(lat) || !hasCoordinates(lng)) return;
            ...
            loading = false;
        })().catch(() => { loading = false; error = 'Peta gagal dimuat. Isi koordinat secara manual.' })"
    >
